...
const categoryId = route.params.cateId;
const categoryData = getFlashcardsByCategory(categoryId);

const increment = () => {
  if (countID.value < categoryData.length - 1) {
    countID.value++;
...
const currentFlashcard = computed(() => categoryData[countID.value]);
</script>
